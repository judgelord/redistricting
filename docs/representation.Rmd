...
# drop name varients
d %<>% select(-contains("first"), -contains("common"), -contains("middle"), -contains("last"), -contains("maiden"))
...
partisanship_agg <- partisanship %>% 
  ...
  summarise(across(contains("_CUM_") | BLACK|WHITE|HISPANIC| ASIAN|AMINDIAN, sum))
```
...
d3 %>%
  select(bioname, district_code, icpsr,
         ...
         # race 
          BLACK, WHITE, HISPANIC, ASIAN, AMINDIAN,
         # partisanship
...
write_csv(member_district, path = here::here("data", "member_district_example.Rdata"))
```
