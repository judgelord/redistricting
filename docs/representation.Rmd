...
by Congress and legislator ICPSR identifier linking to DW-NOMINATE spatial ideology scores based on voting record 
...
- [Facebook friendships](https://github.com/judgelord/redistricting/blob/main/data/WI_congress_facebook_sci_pop.txt)
- [Travel](https://github.com/judgelord/redistricting/blob/main/data/district_flows_safegraph.txt)
...
### Merge in within-district connectivity

```{r}
within <- c %>% filter(from == to)


d2 <- d %>% left_join(within,
                by = c("district_code" = "to")) %>% filter(!is.na(flow_count)) 

# number of speeches
d2 %>% distinct(bioname, cqlabel, flow_count, facebook_sci, speeches_n) %>% kablebox()
```
...
```{r}
d3 <- d2 %>% left_join(partisanship_agg)

d4 <- d3 %>%
  select(bioname, district_code, icpsr,
         # speech
         speeches_n, file,
         # community
...
- [member_district.Rdata](https://github.com/judgelord/redistricting/blob/main/data/member_district_example.Rdata)
- [member_district.csv](https://github.com/judgelord/redistricting/blob/main/data/member_district_example.csv)