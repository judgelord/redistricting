...
nodes <- nodes %>% mutate(label = id, 
                          ...
                          shape = "icon",
                            icon.color = case_when(type =="goal" ~ "black",
                                                   type !="goal" ~ "black"),
                            icon.code = case_when(type == "condition" ~ "f43c", # chess board
                                                  type == "goal" ~ "f24e",# scales  "f05b", # crosshairs
                                                  type == "policy" ~ "f0e3",# gavel
                                                  type == "value" ~ "f4be", # hand with heart
                                                  type == "effect" ~ "f681", # data 
                                                  type == "metric" ~ "f545", # ruler 
                                                  TRUE ~ "f681"),
...
# make directed graph
p <- visNetwork(nodes=nodes, edges=edges, width = "100%") %>% 
